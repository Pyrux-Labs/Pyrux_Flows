...
function CustomTooltip({ active, payload, label }: TooltipEntry) {
	...
	return (
		<div className="rounded-lg border border-border bg-popover px-3 py-2 text-xs shadow-md space-y-1.5 min-w-40">
			...
			<div className="border-t border-border pt-1.5 flex justify-between gap-4">
				...
				<span
					className={`font-mono font-semibold ${positivo ? "text-green-400" : "text-destructive"}`}>
					{positivo ? "+" : ""}
					{formatCurrency(resultado, "USD")}
				</span>
			</div>
		</div>
	);
