...
export function ClientTable({ clients, projectCounts, isLoading, onEdit }: ClientTableProps) {
  const updateClient = useUpdateClient();
  const { data: sectors = [] } = useSectors();
  const { data: allContacts = [] } = useContacts();
  const { sortColumn, sortDirection, handleSort, dir } = useTableSort<SortColumn>("name");
  ...
  const { isActive, renderCell } = useInlineEdit<Client, "name" | "phone">({
    getValue: (item, field) => item[field],
    onCommit: (id, field, value) =>
      updateClient.mutateAsync({ id, payload: { [field]: value } }),
    requiredFields: ["name"],
  });
  ...
  }, [clients, projectCounts, sortColumn, dir, sectors, contactsByClientId]);
          ...
          {sorted.map((client) => {
            ...
            return (
              <TableRow key={client.id} className="hover:bg-secondary/50">
                ...
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start">
                      <DropdownMenuItem
                        onClick={() => updateClient.mutate({ id: client.id, payload: { sector: null } })}
                      ...
                      {sectors.map(({ id, label }) => (
                        <DropdownMenuItem
                          key={id}
                          onClick={() => updateClient.mutate({ id: client.id, payload: { sector: id } })}
                ...
                {/* Contacto */}
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <ContactsCell entityId={client.id} entityType="client" />
                </TableCell>
